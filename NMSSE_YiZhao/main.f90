...
!  write(*, '(i4, 2f14.7)') time_steps, interval_time, total_time
  ...
  allocate(psi(time_steps, N_basis))
  ...
  do i = 1, n_traj_per_para
    ...
    call initialphi()
    call solve_MSSE(time_steps, total_time, N_basis, psi0, third_term_NM, psi(:, :))
    if(my_id /= 0) then
      call MPI_SEND(psi(:, :), time_steps * N_basis, MPI_DOUBLE_COMPLEX, 0, i, MPI_COMM_WORLD, ierr)
    else
      write(33, *) i
      do k = 1, time_steps; do l = 1, N_basis
        pdensity(k, l) = pdensity(k, l) + density(psi(k, l)) / (n_traj_per_para * num_procs)
        total_density(k) = total_density(k) + density(psi(k, l)) / (n_traj_per_para * num_procs)
      end do; end do
      do j = 1, num_procs - 1
        call MPI_RECV(psi(:, :), time_steps * N_basis, MPI_DOUBLE_COMPLEX, j, i, MPI_COMM_WORLD, stat, ierr)
        do k = 1, time_steps; do l = 1, N_basis
          pdensity(k, l) = pdensity(k, l) + density(psi(k, l)) / (n_traj_per_para * num_procs)
          total_density(k) = total_density(k) + density(psi(k, l)) / (n_traj_per_para * num_procs)
        end do; end do
