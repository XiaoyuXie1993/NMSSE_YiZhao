...
subroutine discretization()
  ...
  do i = 1, N_basis
    do j = 1, N_omega
      omega = j * interval_omega
      SP(i, i, j) = pi * 0.5d0 * alpha * omega * dexp(- omega / omega_c)
...
subroutine thirdterm_NM()
  ...
  call expansion_Hamiltonian_complex(N_basis, H0, interval_time, U_plus)
  call expansion_Hamiltonian_complex(N_basis, H0, -interval_time, U_minus)
...
!      end if
!      stop
      tmp2 = tmp2 * cdexp(dcmplx(0.0d0, -omega * time)) * dcmplx(0.0d0, -1.0d0) / hbar
